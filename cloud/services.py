...
def _asset_renewal_active_server_asset(order: CloudServerOrder | None) -> CloudAsset | None:
    asset = _order_primary_asset(order)
    if not asset or getattr(asset, 'kind', None) != CloudAsset.KIND_SERVER:
        return None
    if _is_unattached_static_ip_asset(asset):
        return None
    if not str(getattr(asset, 'instance_id', '') or getattr(order, 'instance_id', '') or '').strip():
        return None
    if getattr(asset, 'is_active', True) is False:
        return None
    if getattr(asset, 'status', None) in _INACTIVE_ASSET_STATUSES:
        return None
    return asset
