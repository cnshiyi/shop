...
def forwards(apps, schema_editor):
    ...
    for plan in CloudServerPlan.objects.exclude(provider_plan_id='').iterator():
        ...
        if not (matched_price and str(matched_price.config_id or '').strip()):
            continue
        target_config_id = matched_price.config_id
        exists_conflict = CloudServerPlan.objects.filter(
            provider=plan.provider,
            region_code=plan.region_code,
        ...
        if exists_conflict:
            continue
        plan.config_id = target_config_id
        plan.save(update_fields=['config_id'])


class Migration(migrations.Migration):
    dependencies = [
        ('cloud', '0013_reformat_config_id_short_sequence'),
    ]
