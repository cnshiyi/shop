...
def _first_resolved_asset(queryset, ordering):
...
def _resolve_asset(instance_name, instance_arn, public_ip, order, account=None, region_code=''):
    ...
    if public_ip:
        ...
        if order:
            ...
            order_ip_asset = _first_resolved_asset(order_asset_queryset.filter(ip_q), _asset_resolve_ordering(public_ip))
            if order_ip_asset:
    ...
    if order and direct_candidates:
        ...
        if region_code:
            order_asset_queryset = order_asset_queryset.filter(region_code=region_code)
        order_asset = _first_resolved_asset(order_asset_queryset.filter(direct_candidates), _asset_resolve_ordering())
        if order_asset:
            return order_asset
    if direct_candidates:
        asset = _first_resolved_asset(base_queryset.filter(direct_candidates), _asset_resolve_ordering())
        if asset:
            return asset
        if account:
            deleted_asset = _first_resolved_asset(CloudAsset.objects.filter(lookup).filter(direct_candidates).filter(status__in=_SYNC_EXCLUDED_ASSET_STATUSES), ['-updated_at', '-id'])
            if deleted_asset:
...
def _resolve_server(instance_name, instance_arn, public_ip, order, account=None, region_code=''):
    return _resolve_asset(instance_name, instance_arn, public_ip, order, account, region_code)
...
def _sync_order_deleted_from_cloud(order, old_public_ip, *, source: str = 'AWS 同步校验', asset=None):
    ...
    instance_name = str(getattr(asset, 'instance_id', None) or order.instance_id or order.server_name or '').strip()
    note = f'{source}: 云端未找到实例/IP，订单随同步链标记删除；IP={old_public_ip or "缺失"}；Asset={asset_id or "-"}；实例={instance_name or "-"}；时间={now.isoformat()}。'
...
# 功能：提供 AWS 资产同步 的内部辅助逻辑，供同模块流程复用。
def _delete_asset_missing_in_aws(asset, *, old_public_ip, instance_name, order, stdout):
    ...
    if order:
        _sync_order_deleted_from_cloud(order, old_public_ip, source='AWS 同步资产校验删除', asset=asset)
    record_cloud_ip_log(
        event_type='deleted',
        order=order,
        asset=asset,
...
# 功能：提供 AWS 资产同步 的内部辅助逻辑，供同模块流程复用。
def _mark_asset_missing_confirmation_in_aws(asset, *, old_public_ip, now_iso, stdout=None) -> bool:
...
def _mark_ip_retained_as_unattached(public_ip, static_ip_name, retained_order, account, region, note, now, release_at=None):
    ...
    for related in CloudAsset.objects.filter(lookup).exclude(provider_status__contains='未附加固定IP'):
        ...
        if retained_order and not related.order_id:
            ...
            related.user = related.user or retained_order.user
...
def _mark_deleted_when_missing_in_aws(region, existing_instance_names, existing_public_ips, stdout, account=None, asset_queryset=None):
    ...
    for asset in queryset:
        ...
        if not _mark_asset_missing_confirmation_in_aws(asset, old_public_ip=old_public_ip, now_iso=now_iso, stdout=stdout):
            continue
...
# 命令类：封装 AWS 资产同步 的 Django 管理命令。
class Command(BaseCommand):
    help = '同步 AWS Lightsail 服务器到统一云资产表'

    # 功能：处理 AWS 资产同步 中的 add arguments 业务流程。
    ...
    def handle(self, *args, **options):
        ...
        # 功能：处理 AWS 资产同步 中的 target matches 业务流程。
            ...
            for region in regions:
                ...
                while True:
                    ...
                    for item in instances:
                        ...
                        expires_at = order_asset_expiry(order) if order else None
                        order_user = None
                        ...
                        rebound_to_order = bool(rebound_unattached_ip and order and order_asset_expiry(order))
                        if asset:
                ...
                for item in static_ip_cache:
                    ...
                    if retained_order:
                        order_note = (
                            ...
                            f'服务到期={order_asset_expiry(retained_order).isoformat() if order_asset_expiry(retained_order) else "-"}；'
