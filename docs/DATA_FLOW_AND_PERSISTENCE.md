...
- TRONGrid / 链上接口
  - `orders/payment_scanner.py:1`
  - `cloud/resource_monitor.py:1`
- AWS Lightsail / boto3
  - `cloud/services.py:1`
  - `cloud/lifecycle.py:19`
  - `cloud/management/commands/sync_aws_assets.py:1`
- 阿里云资产同步
...
- `cloud/api_assets.py`：云资产列表、风险摘要和资产载荷辅助；服务器视图来自 `cloud_asset`
- `cloud/api_asset_snapshots.py`：云资产列表快照刷新、快照搜索、风险计数、分页和分组；落库到 `cloud_asset_dashboard_snapshot`
- `cloud/api_asset_edit.py`：云资产详情、人工编辑、自动续费开关和后台删除；负责同步未附加固定 IP 删除计划、清理同资源残留记录并写入详细状态日志
- `cloud/api_orders.py`：云服务器订单列表、详情、状态更新、删除保护
- `cloud/api_tasks.py`：通知计划、自动续费、旧任务列表
- `cloud/api_monitors.py`：监控地址、链上余额、云资产 IP 日志
- `cloud/api_sync.py`：单条代理状态同步、服务器同步、套餐/价格同步
- `cloud/sync_jobs.py`：云资产批量同步任务运行时、事件流、取消/重试、同步状态和指标 API

结论：后台 API 的读取主链路已经以数据库为主，真实实现现在集中在 `bot/api.py`、`orders/api.py` 和 `cloud/api_*` 域模块。
...
### 3. 链上扫描器读取外部接口 + 数据库
- 从数据库读取待支付订单、充值、云订单：`orders/payment_scanner.py:1`
- 调 TRONGrid 查询交易与资源
- 命中后回写数据库状态
...
- `cloud/api_asset_snapshots.py`
  - 代理列表快照刷新、搜索、风险计数、分页和分组
  - 快照刷新从 `cloud_asset` 构建展示载荷，写入 `cloud_asset_dashboard_snapshot`
- `cloud/api_asset_edit.py`
  - 云资产详情读取、人工编辑、自动续费开关和后台删除
  - 人工刷新未附加固定 IP 删除计划时，同步更新同订单/同资源残留记录的到期时间
  - 后台删除代理记录时，同时清理同订单/同资源残留记录，清空订单云资源绑定，并写入 `CloudIpLog` 与结构化 logger
- `cloud/api_orders.py`
  - 云订单状态流转、云订单详情保存、云订单删除保护
- `cloud/api_tasks.py`
  - 通知计划刷新、通知开关、通知文本覆盖、自动续费手动执行
- `cloud/api_sync.py`
  - 单条代理状态同步、服务器同步、套餐/价格同步

结论：后台 API 仍是核心写入面，真实写入逻辑现在集中在 `bot/api.py`、`orders/api.py` 和 `cloud/api_*` 域模块。
...
### 3. 链上扫描器写数据库
`orders/payment_scanner.py` 会在链上支付命中后写回商品订单、充值、云订单等状态。

`cloud/resource_monitor.py` 会更新监控资源数据，包括 `AddressMonitor` 状态与资源快照。
...
- 服务器兼容视图来自 `cloud_asset`，不再同步独立 `cloud_server` 镜像表

### 5. 管理命令写数据库
- 手工录入/修正统一云资产：`cloud/management/commands/upsert_cloud_asset.py:1`
- 同步 AWS 资产：`cloud/management/commands/sync_aws_assets.py:1`
- AWS / 阿里云同步写入 `cloud_asset`，不再维护独立 `cloud_server` 镜像表
- 后台手动同步通过 `cloud_asset_sync_job` 入队并由 `process_cloud_asset_sync_jobs` worker 异步执行；任务状态、进度、当前步骤、worker 心跳、取消请求、错误、警告与日志摘要会持续写回数据库，完整时间线写入 `cloud_asset_sync_job_event`，完成后再刷新 `cloud_asset_dashboard_snapshot`
- 同步任务由 `cloud/sync_jobs.py` 串行执行账号/资产子任务，不再在线程池中并发写任务状态；这让 `progress_current`、事件顺序、取消请求和 worker heartbeat 更容易排障
- `cloud_asset_sync_job_event` 不使用外键，只用 `job_id` 数字索引关联任务，保证详细日志写入失败或锁等待不会反向阻塞同步任务状态更新
- 事件表按运维命令 `prune_cloud_sync_job_events --days 90 --keep-per-job 500` 清理；事件是排障审计数据，不是业务主状态
- 同步任务若只入队但没有 worker 运行，会长期停留在 `queued`，这是可观测状态，不是静默丢失
...
写入方式：
- 在 `orders/payment_scanner.py`
- `cloud/resource_monitor.py`
- `cloud/management/commands/sync_aws_assets.py`
- `cloud/lifecycle.py`
- `bot/api.py`、`orders/api.py`、`cloud/api_*` 域模块以及相关 service 中调用第三方接口的位置
  统一补日志写入
...
写入方式：
- 在 `cloud/resource_monitor.py` 检测到资源变化后写入
