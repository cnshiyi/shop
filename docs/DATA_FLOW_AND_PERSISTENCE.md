...
- 云服务器业务
  ...
  - `cloud.CloudAssetSyncJob`（后台代理同步任务状态、进度与结果）
...
- 后台手动同步通过 `cloud_asset_sync_job` 入队并由 `process_cloud_asset_sync_jobs` worker 异步执行；任务状态、进度、当前步骤、错误、警告与日志摘要会持续写回数据库，完成后再刷新 `cloud_asset_dashboard_snapshot`
