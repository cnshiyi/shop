...
- 前端只读观察：`/Users/a399/Desktop/data/vue-shop-admin/apps/web-antd/src` 未检出旧 `/api/dashboard` 调用，业务页面主要从 `#/api/admin` 导入；前端文档中仍有旧 `/api/dashboard` 说明残留，未在本后端仓库修改。

## 最近验证

- `UV_CACHE_DIR=/private/tmp/uv-cache-shop PYTHONDONTWRITEBYTECODE=1 uv run python manage.py check` 通过。
- `UV_CACHE_DIR=/private/tmp/uv-cache-shop PYTHONDONTWRITEBYTECODE=1 uv run python -m py_compile shop/urls.py shop/admin_urls.py shop/auth_urls.py bot/tests.py cloud/tests.py cloud/tests_task_center.py cloud/services.py cloud/lifecycle.py cloud/lifecycle_tasks.py` 通过。
...
- 前端文档仍有旧 `/api/dashboard` 说明残留；源码只读扫描未发现旧调用。

## 下一步

- 继续按固定巡检清单监控机器人返回链、云资产生命周期唯一到期事实、后台任务中心状态统计和云资产同步 worker 可观测性。
- 后续若切到前端仓库，可清理前端文档里的旧 `/api/dashboard` 描述。
