...
- 时间：2026-06-03 19:01 CST
- 状态：已完成固定巡检，并修复通知计划状态统计把失败重试误归到队列状态的问题。
...
- 红线关键字扫描未发现运行代码恢复订单到期字段、旧计划快照、旧退款函数名或把迁移截止时间写回资产到期事实；剩余命中为资产侧唯一到期事实、固定 IP 回收时间同步或 `_asset_expires_at` 临时属性。
- 废弃 app 目录扫描无输出。
