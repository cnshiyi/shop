...
## 2026-06-08 16:13 代理列表全部风险标签压测与慢页优化

### 背景

继续当前会话自动巡检。上一轮已完成生命周期计划和通知计划专项验证，本轮按用户要求覆盖代理列表所有标签，不只测试“全部”。

### 问题

真实 MySQL、代理列表 IP 视图、非分组、`page_size=20` 下，全标签数据真实性对账通过，但多个风险标签深页明显超过 `2s`：
...
慢点来自风险标签默认排序仍使用通用列表排序：

```text
risk_rank, asset_due_sort_null_rank, asset_due_sort_at, -sort_order, -asset_id
```

这些标签已有更适合的组合索引，但原排序没有完全复用。

本轮还发现一个数据库结构红线：

- `cloud_asset_dashboard_snapshot` 当前已经达到 MySQL 单表 `64` 个索引上限。
- 直接新增 7 个风险标签列表分页索引时，`migrate cloud 0064` 失败：
  - `Too many keys specified; max 64 keys allowed`
- 检查确认没有半截新索引残留。

因此没有继续堆索引，改为调整排序策略复用已有索引。
...
- `normal`、`due_soon`、`expired`、`unattached_ip`、`abnormal`、`unbound_user`、`unbound_group` 默认排序改为：
  - `asset_due_sort_null_rank`
  - `asset_due_sort_at`
  - `group_user_label`
  - `group_user_key`
  - `-asset_id`
- `shutdown_disabled`、`auto_renew_off` 默认排序改为：
  - `group_telegram_key`
  - `group_telegram_label`
  - `-asset_id`
- 显式到期排序保持原契约。
- 原未绑定标签排序测试扩展为全风险标签排序测试：
  - `test_cloud_assets_risk_ordering_uses_existing_page_indexes`

### 数据对账与性能

真实 MySQL 重跑全部标签，第 `1` 页、第 `2` 页、深页和末页候选页均完成 API/数据库同排序对账，未发现丢数据或重复。
...
- `6` 条通过。
- 继续覆盖通知复制并发隔离、钱包直付/补付同时执行，以及 `60` 路批量后台任务。
...
- 继续执行当前会话自动巡检目标。
- 上一轮代理列表标签压测后，本轮按下一步重点巡检生命周期计划页、任务中心和通知页。
- 本轮只做可逆配置切换和页面展示验证，不执行真实关机、删机、释放 IP、支付、链上广播或业务数据删除。

### 发现

- 前端计划页把“关机计划”和“删除计划”放在同一个 `serverPlanSections` 循环里渲染。
- “计划状态”和“执行状态”单元格原本通过 `serverPlanSwitchField(column.key)` 推断当前表应该使用哪个总开关。
- 但这两个单元格的列名分别是 `plan_state_label` 和 `execution_status`，不包含删除计划字段。
- 因此在“删除计划”表里，状态/执行文案会落到默认 `shutdown_enabled`，删除服务器总开关关闭时可能仍按关机总开关展示。
- 后端接口本身已经区分 `cloud_server_shutdown_enabled`、`cloud_server_delete_enabled`、`cloud_ip_delete_enabled`，问题在前端展示口径。
...
- `关机计划` section 增加 `switchField: 'shutdown_enabled'`。
- `删除计划` section 增加 `switchField: 'server_delete_enabled'`。
- “计划状态”和“执行状态”统一用 `section.switchField` 计算有效总开关、阻断原因和执行文案。
- 单项开关列仍使用列名保存 `shutdown_enabled/server_delete_enabled`，没有改变资产级开关写入行为。
...
结论：本轮开关验证没有导致生命周期计划、任务中心或通知计划统计漂移。
...
## 2026-06-08 16:58 分组标签深页巡检与前端 Typography warning 修复

### 背景

本轮继续执行当前会话不少于 4 小时的自动巡检目标。上一轮已修复分组分页空表问题，本轮继续覆盖更多代理列表分组标签深页和末页，同时把机器人多任务高并发保持为固定回归。
...
定位到代理列表表格里多个 `TypographyParagraph` 同时使用 `ellipsis` 和子文本。
...
- 本轮未执行真实云资源创建、关机、删机、释放 IP、换 IP、真实支付、链上广播、生产发布或删除业务数据。
- 本轮未打印密钥、Telegram session、TOTP、支付密钥、云厂商密钥或完整代理链接。
## 2026-06-08 18:45 CST 生命周期计划固定 IP 保留态与 10 万级分页复测

### 本轮背景

- 继续执行当前会话自动巡检目标。
- 用户明确调整压测范围：不再测试百万级，十万级即可。
- 本轮重点继续放在生命周期计划、IP 删除计划/历史分离、真实前端显示、分页对账和机器人高并发。

### 本轮发现

- 计划页装饰层会把“实例已删除 + 固定 IP 保留”的行标记为 `completed/等待IP回收`。
- 这个判断对服务器删除计划行正确，但对 IP 删除计划行不正确：固定 IP 仍然需要释放，不能显示成完成态。
- 这会导致用户看到的 `IP 删除计划` 和 `IP 删除历史` 边界变模糊。

### 本轮修复

- `bot/api.py`
  - `decorate_plan_item` 中“实例已删除 + 固定 IP 保留”的完成态判断改为只适用于非 IP 删除计划行。
  - IP 删除计划行继续展示为固定 IP 未附加、待回收计划。
- `cloud/lifecycle_plan_queries.py`
  - 新增 `retained_unattached_ip_q()`，统一识别固定 IP 保留中的未附加 IP。
  - 固定 IP 保留中的资产进入 IP 删除计划，不进入 IP 删除历史。
  - 已删除或云端不存在的未附加 IP 历史查询排除固定 IP 保留中资产。
- `cloud/tests.py`
  - 增加手动删机且固定 IP 保留后进入 IP 删除计划的回归测试。
  - 增加生命周期执行器关机、删机、IP 删除三阶段串行推进测试，防止同一轮连跳多个破坏性阶段。
...
通知计划当前数据未达到 10 万分组，本轮只做现有全量范围内对账：
...
- 首次计划接口返回关机计划 `1979990`、IP 删除计划 `500000`、IP 删除历史 `520010`。
- 点击下一页后，关机计划返回 page `2`、loaded `50`。
- 控制台 error/warning：`0`。
- 业务 API 失败：`0`。
...
## 2026-06-08 20:54 CST 机器人高并发真机测试前置巡检与后台任务隔离

### 本轮背景

- 继续执行当前会话自动巡检目标。
- 通知计划页已完成后，转入剩余未压测项之一：机器人多任务高并发真机点击。
- 本轮先核查机器人进程、项目 Telegram 登录账号、bot token 连通性和安全测试边界。
- 本轮不执行真实支付、链上广播、真实云资源创建/删除或生产发布。
...
- 数据库里有 `1` 个 Telegram 登录账号：
  - 存在 session。
  - 原状态为 `listener_error`。
  - 重新检查 session 时 Telegram MTProto 连续 6 次连接超时，状态仍为 `listener_error`。
- bot token 已配置，但单独 `getMe` 连通性检查返回 `TelegramNetworkError`。
- 因 Telegram 网络不可达，本轮没有完成真实 Telegram 点击或高并发真实消息压测。

### 暴露问题

首次启动 `run.py bot` 时，虽然 Bot API 获取身份超时，但进程继续启动了后台任务：
...
- `bot/runner.py`
  ...
  - 关闭流程只取消实际创建过的后台任务，避免禁用模式关闭时报空任务。
- `bot/tests.py`
...
- 本轮已经把“机器人交互巡检”和“后台生命周期/支付/通知任务”隔离开。
- 后续网络恢复后，可以用 `SHOP_BOT_BACKGROUND_TASKS_ENABLED=0` 安全启动 bot 做真机点击，不会误触发后台生命周期扫描。
...
## 2026-06-08 22:00 CST 生命周期计划过期状态修复与真实前端复测

### 本轮背景

- 继续执行当前会话不少于 4 小时的自动巡检目标。
- 上一轮生命周期计划页真实库巡检发现：部分已过期、仍未执行的关机/删机计划，在服务端分页路径中被统一标为 `scheduled_future`，导致页面显示为未来计划，而不是待执行。
- 本轮只修复计划状态口径，不执行真实关机、删机、IP 释放、支付或链上广播。

### 修复内容

- `bot/api.py`
  - `_server_lifecycle_plan_page_items()` 增加按计划时间计算队列状态的逻辑。
  - 关机计划按 `suspend_at` 判断：
    - 已到时间：`due_now / 待执行`
    - 7 天窗口内：`within_window / 计划中`
    - 更远未来：`scheduled_future / 计划中`
  - 删除计划按 `delete_at` 判断同样状态。
- `cloud/tests.py`
  - 新增 `test_lifecycle_plans_server_page_marks_overdue_delete_as_due_now`，验证已过期删除计划不能误标为未来计划。
  - `test_lifecycle_plans_include_future_server_plan_item` 显式开启删除总开关，避免测试依赖默认配置。

### 真实库对账

通过真实库直接调用 `_server_lifecycle_plan_page_items()` 验证：
...
首次打开 `http://127.0.0.1:5666/admin/tasks/plans` 时，前端进入页面前请求 `/api/admin/user/info` 返回 `502`。

定位结果：

- 前端 dev server 正常。
- `127.0.0.1:8000` 后端不在，导致前端代理失败。

处理：

- 重新启动后端：`uv run python manage.py runserver 127.0.0.1:8000`
- 重新生成临时后台登录态，写入 `/private/tmp/shop-plans-storage-state.json`，未打印 session 内容。
- 使用系统 Chrome 打开真实前端计划页。
...
- 巡检 JSON：`/private/tmp/shop-lifecycle-plan-status-front.json`
...
- 生命周期计划页服务端分页状态口径已修复并通过真实前端复测。
- 本轮没有执行真实支付、链上广播、真实云资源创建/删除、生产发布或删除业务数据。
- 下一轮继续处理机器人真机高并发和真实生命周期闭环剩余项。

## 2026-06-08 22:08 CST 生命周期计划深分页与真实前端翻页巡检

### 本轮背景

- 继续执行当前会话自动巡检目标。
- 上一轮修复了已过期关机/删机计划被误标为未来计划的问题。
- 本轮继续复查生命周期计划页深分页，重点验证前端翻页是否会丢数据、串页或与数据库口径不一致。
- 本轮不执行真实关机、删机、IP 释放、支付或链上广播。
...
- 巡检 JSON：`/private/tmp/shop-lifecycle-plan-pagination-front.json`
...
## 2026-06-08 22:13 CST 机器人真机高并发压测前置连通性复查

### 本轮背景

- 继续执行当前会话自动巡检目标。
- 剩余未完成项之一是机器人真机点击和多任务高并发压测。
- 上一轮已经增加 `SHOP_BOT_BACKGROUND_TASKS_ENABLED=0`，可以隔离机器人交互与后台生命周期/支付/通知任务。
- 本轮只复查连通性，不发送真实 Telegram 消息，不启动后台生命周期任务。
...
- Telegram API 凭据存在。
- 使用项目现有 `_telegram_check_session()` 检查登录账号 session。
- `30s` 超时窗口内失败。
- 错误类型：`TimeoutError`
- 未打印 session。
...
- Telegram Bot API 和 MTProto 恢复可连后，继续执行机器人真机点击和多任务高并发压测。
- 真实云资源创建后的完整关机、删机、IP 释放闭环仍需继续在授权范围内逐项验证。

## 2026-06-08 22:18 CST 生命周期闭环开关矩阵与任务队列只读巡检

### 本轮背景

- 继续执行当前会话自动巡检目标。
- 用户明确关注生命周期创建服务器、关机、删机、IP 删除，以及各级开关联动。
- 本轮只做读侧矩阵核查，不执行真实关机、删机或 IP 释放。
