...
## 2026-06-12 22:25 CST AWS 固定 IP 生命周期和删除记录纠正真机测试

- 状态：通过；测试中发现并修复同步链路缺口。
- 授权：用户明确授权真实云资源成本，允许使用 AWS Lightsail 最小规格测试，并允许创建、绑定、解绑、删除测试服务器和固定 IP；用户同时允许本轮临时越过生命周期冻结红线。
- 云厂商：AWS Lightsail
- 云账号：后台 AWS 云账号 `#55`
- 地区：`ap-southeast-1`
- 套餐：`nano_3_0`
...
- 本地测试资产：`CloudAsset #41`、`CloudAsset #42`

### 真机验证范围

1. 创建真实 Lightsail 测试实例，并分配、绑定固定 IP。
2. 定向同步后确认代理列表快照显示 `服务器`，并进入服务器生命周期计划。
3. 对仍附加实例的 IP 调用未附加 IP 删除执行器，确认被拒绝，错误为“该 IP 仍有关联实例，不能按未附加 IP 删除”。
4. 将本地服务器资产人为标记为 `deleted` 且 `public_ip` 清空，仅保留 `previous_public_ip`，再只按公网 IP 定向同步，确认云端仍存在时可恢复为 `running` 服务器。
5. 真实解绑固定 IP，再按 IP 同步，确认同一资产转为未附加 IP：
   - `instance_id` 清空。
   - `status=unknown`。
   - 代理列表快照显示 `未附加IP`。
   - `CloudAsset.actual_expires_at` 从服务器到期时间切换为未附加 IP 删除计划时间。
6. 对云端仍存在的未附加 IP 人为写入回收日志，确认仍留在未附加 IP 删除计划，不混入 IP 删除历史。
7. 将未附加 IP 人为标记为 `deleted` 且清空 `public_ip`，再按 IP 同步，确认云端仍存在时恢复到未附加 IP 删除计划，并从历史中排除。
8. 真实释放未附加固定 IP，确认本地资产标记为 `deleted`、公网 IP 清空、历史 IP 保留在 `previous_public_ip`，并进入 IP 删除历史。
9. 释放后再次按资产/IP 同步，确认云端已不存在时不会被错误恢复。

### 补充回归覆盖

- 用户追加要求确认“IP 是附加状态，但混入了删除记录”的场景。
- 本轮新增 Fake Lightsail 回归测试覆盖：
  - 云端 StaticIp 仍 attached 到实例。
  - 本地资产已混入 `未附加固定IP-已到期删除` 状态和 `recycled` 日志。
  - 只按公网 IP 同步后，资产恢复为服务器，移出未附加 IP 删除计划和 IP 删除历史。

### 修复结论

- AWS 同步现在可以在只给 `--public-ip` 的情况下，用 `previous_public_ip` 找回同账号同地区已误标 `deleted/terminated` 的服务器资产。
- 固定 IP 从未附加重新绑定实例后，会清理旧的“未附加/已删除/已释放”识别备注，避免继续被计划页归为未附加 IP。
- IP 删除历史查询会排除已经被云同步纠回的活跃服务器资产日志，避免“云端仍附加/仍存在”的 IP 继续混在删除历史里。

### 清理结果

- 两轮真实测试实例均已提交删除。
- 两轮真实测试固定 IP 均已释放或确认不存在。
- 清理后只读复核：
  - 测试实例不存在。
  - 测试固定 IP 不存在。
- 本地遗留测试资产已标记为 `deleted/is_active=False` 或随真实释放进入删除历史。

### 风险说明

- 本轮执行真实 AWS 创建实例、分配固定 IP、绑定固定 IP、解绑固定 IP、释放固定 IP 和删除实例；操作前已获用户明确授权。
- 为验证真实释放执行器，临时把未附加 IP 删除时间窗口调整到当前分钟，测试完成后已恢复原配置。
- 未记录完整实例名、固定 IP 名、公网 IP、ARN、密钥、密码或代理 secret。

## 2026-06-12 22:34 CST AWS 附加固定 IP 混入删除记录真机补测
...
- 套餐：`nano_3_0`
- 镜像：`debian_12`
- 测试资源前缀：`codex-attached-mix-********`
- 公网 IP 脱敏：`18.136.xxx.xxx`
- 本地测试资产：`CloudAsset #43`

### 测试过程

1. 真实创建一台 Lightsail 最小规格测试实例。
2. 真实分配一个固定 IP，并绑定到该实例。
3. 按实例名定向运行 AWS 同步，确认本地资产和代理列表快照显示为 `服务器`，并进入服务器生命周期计划。
4. 人为制造“IP 云端仍是附加状态，但本地混入删除记录”的状态：写入回收日志、标记资产为 `deleted/is_active=False`、清空 `public_ip`、仅保留 `previous_public_ip`、清空 `instance_id`、写入 StaticIp 风格资源标识和 `未附加固定IP-已到期删除` 状态。
5. 确认混入状态下该资产会进入 IP 删除历史。
6. 只按公网 IP 定向运行 AWS 同步，不传实例名。
7. 验证同步按云端 attached 事实恢复资产。

### 验证结果

- 同步后同一资产恢复为 `status=running`、`is_active=True`。
- 同步后 `public_ip` 和 `instance_id` 均恢复。
- 同步后资源标识恢复为 Instance 类型。
- 代理列表快照恢复显示 `服务器`。
- 未附加 IP 删除计划中不存在该资产。
- IP 删除历史日志中不再显示该资产。
...
## 2026-06-13 13:06 CST Bot IP 查询状态续费真机测试

- 状态：通过。
- 授权：用户明确授权使用后台 AWS 测试账号 `#55` / `ap-southeast-1` 创建 nano 实例和固定 IP，执行关机、解绑、删机、同步、机器人查询验证，并在结束后释放资源。
- 云厂商：AWS Lightsail
- 云账号：后台 AWS 云账号 `#55`
- 地区：`ap-southeast-1`
- 套餐：`nano_3_0`
- 镜像：`debian_12`
- 测试资源前缀：`codex-botstate-********`
- 公网 IP 脱敏：`52.220.xxx.xxx`
- 本地测试订单：`CloudServerOrder #26`
- 本地测试资产：`CloudAsset #49`
- Telegram 测试账号：项目内已登录账号，用户 ID 已脱敏记录在本地临时 JSON；本报告不记录 session 或 token。

### 测试过程

1. 真实创建一台 Lightsail nano 测试实例。
2. 真实分配一个固定 IP，并绑定到该实例。
3. 在本地创建关联 `CloudServerOrder` 和 `CloudAsset`，到期事实写入 `CloudAsset.actual_expires_at`。
4. 真实关机实例，并用 bot 发送脱敏 IP 查询，验证关机服务器显示 `续费IP` 和 `开机`。
5. 实际点击 `开机` 回调，bot 进入管理员开机路径，AWS 实例恢复为 `running`。
6. 将本地记录置为旧机保留/等待删除状态，bot 管理员查询验证仅显示 `续费IP`、`修改时间` 和返回入口，不显示 `开机`、自动续费、换 IP、重装、修改配置；非管理员视角本轮另行补回归测试，不能显示 `修改时间`。
7. 实际点击旧机保留状态的 `续费IP`，验证进入续费支付入口；未点击钱包支付或地址支付。
8. 真实解绑固定 IP，将本地记录置为未附加固定 IP 且尚未删除，bot 查询验证显示资产续费入口 `cloud:aa:renew`，不显示订单续费入口 `cloud:renew`。
9. 实际点击未附加固定 IP 的 `续费IP`，验证进入恢复新服务器套餐选择页；未继续提交代理链接、未创建支付订单。
10. 真实释放固定 IP，将本地记录置为已到期删除，bot 查询验证返回“未查询到可续费的有效 IP 记录”，无续费按钮。

### 验证结果

- 关机服务器：查询结果包含 `续费IP` 和 `开机`，点击 `开机` 后 AWS 状态恢复为 `running`。
- 旧机保留/等待删除服务器：查询结果保留 `续费IP`，隐藏 `开机` 和自动续费；点击 `续费IP` 进入续费支付入口。
- 未附加但未删除固定 IP：查询结果显示资产续费入口，点击后进入恢复新服务器套餐选择页。
- 已释放/已删除未附加 IP：查询结果为空，不显示任何续费入口。
...
- 本地测试订单和资产已标记为 `deleted`，公网 IP 清空，仅保留脱敏历史说明。

### 结论

- Bot IP 查询状态按钮口径通过真机验证：关机服务器可开机，旧机保留服务器可续费，未附加未删除固定 IP 可走资产续费，已删除未附加 IP 不可续费。
- 本轮只验证按钮展示、回调入口和云端状态变化；未执行真实续费付款。
- 未记录完整实例名、固定 IP 名、公网 IP、ARN、密钥、密码、Telegram session、bot token 或代理 secret。
