...
class TronGridFallbackTestCase(SimpleTestCase):
    def test_bot_tron_address_summary_uses_runtime_base_url(self):
        ...
        with patch('bot.tron_queries.get_runtime_config', return_value='https://tron.internal.example'), \
            patch('bot.tron_queries.build_trongrid_headers', new=AsyncMock(return_value={'accept': 'application/json'})), \
            patch('bot.tron_queries.httpx.AsyncClient', FakeClient):
...
class RetainedIpRenewalUiTestCase(SimpleTestCase):
    ...
    def test_cloud_server_created_text_includes_socks5_proxy_link(self):
        ...
        with patch('bot.cloud_texts._bot_text', side_effect=lambda _key, default: default):
