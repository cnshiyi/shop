...
order = CloudServerOrder.objects.create(
    order_no=f"TESTAWSRAW{timezone.now().strftime('%Y%m%d%H%M%S')}",
    user_id=1,
    plan=plan,
...
print('created_order', order.id, order.order_no, plan.id, plan.plan_name, plan.region_code, flush=True)
result = asyncio.run(create_instance(order, f"rawtest-{timezone.now().strftime('%H%M%S')}"))
print(json.dumps({
    'ok': result.ok,
    'instance_id': result.instance_id,
    'public_ip': result.public_ip,
    'login_user': result.login_user,
    'has_password': bool(result.login_password),
    'note': result.note,
