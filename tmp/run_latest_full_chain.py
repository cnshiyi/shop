...
order = CloudServerOrder.objects.filter(order_no__startswith='TESTAWSSSH').order_by('-id').first()
assert order, 'no latest aws ssh test order'
IP = order.public_ip or order.previous_public_ip
assert IP, 'latest aws ssh test order has no ip'
...
channel.exec_command("bash -lc 'whoami; id; apt-get update -y >/dev/null; apt-get install -y ca-certificates curl wget sudo procps iproute2 git make gcc libssl-dev zlib1g-dev >/dev/null 2>&1 || true; printf \"%s\\n\" net.core.default_qdisc=fq net.ipv4.tcp_congestion_control=bbr > /etc/sysctl.d/99-bbr.conf; sysctl --system >/tmp/bbr.out 2>&1 || true; sysctl net.ipv4.tcp_congestion_control; sysctl net.core.default_qdisc'" % '%s')
out = b''
err = b''
while True:
