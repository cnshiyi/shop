...
BOT_TEXTS = {
    ...
    'bot_custom_port_invalid': ('旧端口输入流程已取消，系统会使用默认端口 443 并继续创建任务。\n\n可随时点击底部菜单打断当前输入。', '旧端口输入兼容提示'),
    'bot_custom_context_missing': ('订单上下文已失效，请重新下单。', '自定义下单上下文失效提示'),
    'bot_change_ip_failed': ('更换IP失败，请返回详情页重试。', '更换 IP 失败提示'),
    'bot_set_port_failed': ('订单不存在，无法提交创建任务。', '默认端口创建失败提示'),
    ...
    'bot_custom_port_hint': ('系统将使用默认端口 443，并直接进入服务器创建流程。', '云服务器默认端口提示'),
    ...
    'bot_custom_port_success': ('✅ 已使用默认端口 {port}\n已开始后台创建服务器，我会在完成后主动通知你。', '云服务器默认端口创建提示'),
