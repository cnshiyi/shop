...
BOT_TEXTS = {
    ...
    'bot_monitor_invalid_resource_threshold': ('❌ 请输入有效整数，0 表示只要增加就通知。', '资源阈值非法提示'),
    ...
    'bot_monitor_threshold_prompt_energy': ('请输入新的能量增加阈值（整数）：\n例如 10000；输入 0 表示只要增加就通知。\n\n可随时点击底部菜单打断当前输入。', '能量阈值输入提示'),
    'bot_monitor_threshold_prompt_bandwidth': ('请输入新的带宽增加阈值（整数）：\n例如 500；输入 0 表示只要增加就通知。\n\n可随时点击底部菜单打断当前输入。', '带宽阈值输入提示'),
    ...
    'bot_monitor_energy_threshold_updated': ('✅ 能量增加阈值已更新为 {amount}', '能量阈值更新成功提示'),
    'bot_monitor_bandwidth_threshold_updated': ('✅ 带宽增加阈值已更新为 {amount}', '带宽阈值更新成功提示'),
    ...
    'bot_reinstall_need_main_link': ('当前服务器缺少主代理链接。请直接发送这台服务器的主代理链接，我会先校验 IP、端口和服务器实际密钥；端口必须与系统记录一致，未记录时使用默认端口 443。校验通过后再让你确认是否重建迁移。', '重建迁移缺少主代理链接提示'),
    'bot_resume_init_confirm': ('⚠️ 确认继续初始化？\n\n系统会重新执行 BBR/MTProxy 安装并生成代理链接。', '继续初始化确认提示'),
    'bot_reinstall_confirm': ('⚠️ 确认重建迁移？\n\n系统会新建服务器并迁移固定 IP，主/备用链接保持不变；旧机保留 3 天后进入删除流程。', '重建迁移确认提示'),
    ...
    'bot_reinstall_validate_ok': ('主代理链接校验通过。\n\n⚠️ 确认重建迁移？系统会新建服务器并迁移固定 IP，旧机保留 3 天后进入删除流程。', '重建迁移主代理链接校验成功提示'),
    'bot_reinstall_submitted': ('🛠 已确认{action_text}，后台会{work_text}。预计约 5 分钟，完成后会自动通知你。', '重装/继续初始化已提交提示'),
    'bot_retained_ip_renewal_plan_intro': ('🔄 未附加固定 IP 续费\n\n保留 IP: {ip}\n地区: {region}\n\n请选择要恢复的新服务器套餐。选好后，我会要求你发送旧的主代理链接，用来保持原链接/密钥不变。', '未附加固定 IP 续费套餐页文案'),
    'bot_retained_ip_renewal_plan_footer': ('请选择下面的套餐按钮：', '未附加固定 IP 续费套餐页底部提示'),
    'bot_retained_ip_renewal_link_prompt': ('🔄 未附加固定 IP 续费\n\n已选择套餐: {plan_name}\n保留 IP: {ip}\n\n请直接发送这台服务器旧的主代理链接（tg://proxy?... 或 https://t.me/proxy?...）。\n我会校验 IP、端口和密钥；端口必须与系统记录一致，未记录时使用默认端口 443；校验通过后再生成续费支付订单。', '未附加固定 IP 续费旧链接输入提示'),
    'cloud_auto_renew_failed': ('❌ 自动续费失败\n\nIP: {ip}\n\n{plan_text}\n\n失败原因: {error}\n\n请在关机/删机计划前完成续费；如余额不足，请先充值或联系人工客服处理。', '自动续费失败通知'),
    'cloud_delete_notice': ('⚠️ 云服务器删机提醒\n\nIP: {ip}\n订单号: {order_no}\n\n{plan_text}\n\n请在 {delete_at} 前完成续费，避免服务器实例被删除；不需要提醒可点击下方关闭提醒。', '云服务器删机提醒'),
    'cloud_ip_recycle_notice': ('📦 固定 IP 释放提醒\n\nIP: {ip}\n订单号: {order_no}\n\n{plan_text}\n\n固定 IP 仍在保留中，请在 {ip_recycle_at} 前续费恢复，避免该 IP 被释放；不需要提醒可点击下方关闭提醒。', '固定 IP 释放提醒'),
    'cloud_expiring_notice': ('⏰ 云服务器即将到期\n\nIP: {ip}\n订单号: {order_no}\n\n{plan_text}\n\n请尽快续费，避免后续关机、删机或固定 IP 释放；不需要提醒可点击下方关闭提醒。', '云服务器即将到期提醒'),
    'cloud_suspended_notice': ('⚠️ 云服务器已关机\n\nIP: {ip}\n订单号: {order_no}\n\n{plan_text}\n\n请在 {delete_at} 前完成续费，避免服务器实例被删除；不需要提醒可点击下方关闭提醒。', '云服务器已关机通知'),
    'cloud_instance_deleted_notice': ('🗑 云服务器实例已删除\n\nIP: {ip}\n订单号: {order_no}\n\n{plan_text}\n\n固定 IP 仍在保留中，请在 {ip_recycle_at} 前续费恢复；不需要提醒可点击下方关闭提醒。', '云服务器实例已删除通知'),
    'cloud_ip_retention_ended_notice': ('📦 固定 IP 保留期已结束\n\nIP: {ip}\n订单号: {order_no}\n\n{plan_text}\n\n固定 IP 已到释放时间，如仍需处理请联系人工客服。', '固定 IP 保留期结束通知'),
