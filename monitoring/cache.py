...
async def _write_monitor_entries(address: str, entries: list[dict]):
    r = await get_redis()
    if r is None:
        return
    if entries:
        await r.hset(MONITORS_KEY, address, json.dumps(entries, ensure_ascii=False))
    else:
...
async def remove_monitor_from_cache(address: str, monitor_id: int | None = None):
    ...
    await _write_monitor_entries(address, entries)


async def update_monitor_threshold_in_cache(address: str, currency: str, amount: Decimal, monitor_id: int | None = None):
    r = await get_redis()
    if r is None:
        return
    ...
    if raw:
        ...
        for entry in entries:
            if monitor_id is None or int(entry.get('id') or 0) == int(monitor_id):
        ...
        try:
            ...
            if all_data:
                ...
                for addr, raw in all_data.items():
                    result.setdefault(addr, []).extend(_decode_monitor_entries(raw))
                return result
